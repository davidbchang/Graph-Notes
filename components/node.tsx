...
import { IPoint } from './graph-view-props';

// modified from https://github.com/uber/react-digraph/blob/master/src/components/node.js
export type INode = {
  id: number,
  title: string,
  description: string,
...
function Node({
  ...
  let positionChange = {x: data.x || 0, y: data.y || 0, pointerOffset: null};
  let pChange = useRef();
  ...
  pChange.current = { x: 1, y: 2, pointerOffset: null };
  ...
  const handleMouseOut = useCallback(
    ...
    const handleDragStart = useCallback((event) => {
        ...
        // get start position before dragging
        positionChange.x = position.current.x
        positionChange.y = position.current.y
        
        let posX = position.current.x
        let posY = position.current.y
        pChange.current.x = posX
        pChange.current.y = posY
  ...
  const handleDragEnd = useCallback(
    (event: any) => {
      ...
      // if node was not dragged, then it was clicked
      if (pChange.current.x == position.current.x && pChange.current.y == position.current.y) {
        const onFinally = () =>
  ...
  useEffect(() => {
    const dragFunction = d3
      ...
      .on('start', () => handleDragStart(d3.event))
