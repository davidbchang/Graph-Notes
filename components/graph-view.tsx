...
import GraphControls from 'react-digraph'
...
class GraphView extends React.Component<IGraphViewProps, IGraphViewState> {
  ...
  componentDidMount() {
    ...
    d3.select(this.graphSvg.current)
      ...
      //.on('mousemove', (e) => this.handleDocumentMouseMove(e));
  ...
  componentDidUpdate(prevProps: IGraphViewProps, prevState: IGraphViewState) {
    ...
    // remove old edges
    console.log('prevState.edges, edgesMap:', prevState.edges, edgesMap)
    this.removeOldEdges(prevState.edges, edgesMap);

    // remove old nodes
    console.log('prevState.nodes, prevState.nodesMap, nodesMap:', prevState.nodes, prevState.nodesMap, nodesMap)
  ...
  removeOldNodes(prevNodes: any, prevNodesMap: any, nodesMap: any) {
    const nodeKey = this.props.nodeKey;
    console.log('REMOVEOLDNODES!!!!!', nodeKey)
    // remove old nodes
  ...
  removeEdgeElement(source: string, target: string) {
    ...
    console.log('removeEdgeElement:', id)
    GraphUtils.removeElementFromDom(
      `edge-${id}-container`,
      this.viewWrapper.current
    );
  ...
  deleteNode(selectedNode: INode) {
    ...
    console.log('selectingNode in deleteNode:', selectedNode)
    this.setState({
      componentUpToDate: false,
      hoveredNode: false,
    });
  ...
  deleteEdge(selectedEdge: IEdge) {
    ...
    console.log('selectedEdge in deleteEdge:', selectedEdge)
    this.setState({
      componentUpToDate: false,
    });
    this.props.onDeleteEdge(selectedEdge)
    // remove from UI
    ...
    switch (d.key) {
      ...
      case 'Backspace':
        ...
        if (selected && (selected.nodes?.size || selected.edges?.size)) {
            console.log('BACKSPACED', selected)
  ...
  endDragEdge = (extraState: any = {}, callback: () => void = () => {}) => {
      console.log('@@@ENDDRAGEDGE:', extraState)
    this.setState(
      {
        ...extraState,
        componentUpToDate: false,
...
handleNodeSelected = (
    ...
    console.log('IN HANDLENODESELECTED 1')
    ...
    if (!creatingEdge) {
        ...
        // }
        console.log('IN HANDLENODESELECTED 2:', foundSelectedNode)
  ...
  isArrowClicked(event: any, edge: IEdge | null) {
    const { edgeArrowSize, onArrowClicked } = this.props;
    const eventTarget = event.sourceEvent.target;
  ...
  containZoom(event: any) {
    const stop = event.button || this.isControlKeyPressed(event);

    if (stop) {
      event.stopImmediatePropagation(); // stop zoom
    }
  }
  ...
  handleZoomStart = (event: any) => {
    ...
    if (!this.isArrowClicked(event, edge) || !edge) {
      return false;
    }
  ...
  renderGraphControls() {
    ...
    console.log('viewTransform in render:', viewTransform)
  ...
  render() {
    ...
    // Removed highlight-area https://github.com/uber/react-digraph/blob/master/src/components/graph-view.js
    return (
