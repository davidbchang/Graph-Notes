...
import { IEdge, ITargetPosition } from '../components/edge';
import {
...
export function parsePathToXY(edgePathElement: Element | null) {
  ...
  if (edgePathElement) {
    ...
    if (dArr.length === 4) {
      response.source.x = parseInt(dArr[0]);
      response.source.y = parseInt(dArr[1]);
      response.target.x = parseInt(dArr[2]);
      response.target.y = parseInt(dArr[3]);
    }
  }
